...
On **AWS serverless** infrastructure (provisioned by **Terraform**), the program runs an **ELT** pipeline with **Lambda** images in **ECR**, triggered by **EventBridge**. On finishing, ingestion will automatically trigger transformation, with compaction being scheduled every hour. Raw data snapshots are stored in **S3**, then aggregated using the hierarchical hexagon aggregation method provided by **H3**. Transformed data are stored in **DynamoDB** for low-latency real-time queries, and in **Parquet (S3/Athena)** for historical analysis. Aggregated data are compacted from minutely into hourly Parquet files to improve read efficiency and reduce Athena scan overhead. **API Gateway** and **CloudFront** serve the application. **IAM roles** and scoped resource permissions enforce secure access.
